...
export function calculateInputSheetProgress(assumptions: PropertyAssumptions): number {
  ...
  if (assumptions.purchasePrice && assumptions.purchasePrice > 0) points += 12
  if (assumptions.propertyType) points += 8
  if (assumptions.holdPeriodYears && assumptions.holdPeriodYears > 0) points += 8
  if (assumptions.landPercentage > 0 && assumptions.improvementsPercentage > 0 && 
      Math.abs((assumptions.landPercentage + assumptions.improvementsPercentage) - 100) < 0.01) points += 12
  ...
  // Financing (25 points)
  if (assumptions.financingType === 'cash') {
    points += 25 // Cash deals get full financing points
  } else if (assumptions.financingType === 'dscr' || assumptions.financingType === 'ltv') {
    // Only award points if we have meaningful financing details beyond just type selection
    let financingPoints = 0
    if (assumptions.interestRate > 0) financingPoints += 5
    if (assumptions.loanTermYears > 0) financingPoints += 5
    if (assumptions.amortizationYears > 0) financingPoints += 5
    if (((assumptions.targetDSCR ?? 0) > 0) || ((assumptions.targetLTV ?? 0) > 0) || (assumptions.loanAmount > 0)) financingPoints += 5
    // Award 5 points for financing type selection only when other details are also provided
    if (financingPoints > 0) financingPoints += 5
    points += financingPoints
  }

  // Tax & Exit (20 points)
  if (assumptions.ordinaryIncomeTaxRate > 0) points += 4
  if (assumptions.capitalGainsTaxRate > 0) points += 4
  if (assumptions.depreciationRecaptureRate > 0) points += 4
  if (assumptions.dispositionPriceType === 'dollar' || assumptions.dispositionPriceType === 'caprate') points += 4
  if (assumptions.costOfSaleType === 'dollar' || assumptions.costOfSaleType === 'percentage') points += 4
