...
export async function GET(
  ...
  try {
    ...
    const transactions = await DatabaseService.getExpenseTransactions(propertyId, filters)
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Failed to fetch expense transactions' },
      { status: 500 }
    )
  }
}
...
export async function POST(
  ...
  try {
    ...
    const transaction = await DatabaseService.createExpenseTransaction({
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Failed to create expense transaction' },
      { status: 500 }
    )
  }
}
